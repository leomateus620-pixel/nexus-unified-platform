...
def silos(lod):
    ...
    for e in DATA['elements'][:4]:
        b=Builder();x,_,z=e['position'];g=e['geometry'];r=g['radius'];h=g['bodyHeight'];base=g['baseHeight'];cone=g['coneHeight'];n=96 if lod=='high' else 64
...
b=Builder();road(b,DATA['terrain']['access'],9,'gravel',.085);b.build('TR-02',terrain,next(e for e in DATA['elements'] if e['id']=='TR-02'))
...
e=next(e for e in DATA['elements'] if e['id']=='PV-01');b=Builder();x,_,z=e['position'];g=e['geometry'];b.box('concrete',(x,g['height']/2,z),(g['width'],g['height'],g['depth']))
...
e=next(e for e in DATA['elements'] if e['id']=='EQ-01');b=Builder();x,_,z=e['position'];height=e['geometry']['height']
...
for s in DATA['elements'][:4]:
    sx,_,sz=s['position'];g=s['geometry'];top=g['baseHeight']+g['bodyHeight']+g['coneHeight']+.65
...
e=next(e for e in DATA['elements'] if e['id']=='EQ-02');b=Builder();gx,_,gz=e['position'];cz=DATA['elements'][4]['position'][2]
for dz in [gz-6,gz+9]:
...
b.build(e['id'],equip,e)
e=next(e for e in DATA['elements'] if e['id']=='EQ-03');b=Builder();x,_,z=e['position'];b.cylinder('concrete',(x,.02,z),2.65,.22);b.cylinder('paint',(x,.24,z),e['geometry']['radius'],e['geometry']['height']);b.cylinder('roof',(x,2.44,z),2.35,.16,64,2.2);b.build(e['id'],equip,e)

buildings=collection('buildings');COLS['buildings']=buildings
for e in DATA['elements']:
    if e['category']!='buildings':continue
    b=Builder();x,_,z=e['position'];g=e['geometry'];w=g['width'];d=g['depth'];h=g['height'];rise=g['rise'];main=e['id']=='ED-01'
...
for e in DATA['elements']:
    b=Builder();x,_,z=e['position'];g=e['geometry']
    if e['category']=='silos':
...
for name,co in COLS.items():export(co,name)
# Authoring scene includes anchored vegetation with linked geometry.
planted=collection('vegetation-layout')
for i,t in enumerate(DATA['trees']):
    prototype=prototypes[f"{t['type']}-{t['variant']}"]
...
for hidden in [veg,veg_low,block]:hidden.hide_viewport=True;hidden.hide_render=True
...
bpy.context.scene['source']='Four supplied photographs. Estimated dimensions. No operational integration.'
bpy.ops.file.pack_all()
bpy.ops.wm.save_as_mainfile(filepath=str(SOURCE/'3tentos-reconstruction.blend'),compress=True)
low=silos('low');export(low,'silos-low')
print('RECONSTRUCTION_EXPORT_COMPLETE')
